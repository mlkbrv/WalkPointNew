...
// --- partner-facing ----------------------------------------------------------
// Everything below is for a signed-in `partner` account managing its own
// business. `/v1/business/*` never sees another partner's rows; the server
// scopes by the owner on the token rather than trusting an id from the client.

export type ModerationStatus = "draft" | "pending" | "approved" | "rejected";

export interface ApiOwnCoupon {
  id: string;
  partner_id: string;
  category_id: string | null;
  title: string;
...
export interface ApiCouponDraft {
  title: string;
  description?: string;
  rules?: string;
  cost_coins: number;
...
export interface ApiPartnerStats {
  live_coupons: number;
...
export interface ApiScanPreview {
  voucher_id: string;
  coupon_title: string;
  status: "active" | "used" | "expired";
  cost_paid: number;
...
export interface ApiCouponSales {
  issued: number;
  redeemed: number;
...
export const businessApi = {
  profile: () => api.get<ApiStore>("/v1/business/profile"),
  stats: () => api.get<ApiPartnerStats>("/v1/business/stats"),
  coupons: () => api.get<ApiOwnCoupon[]>("/v1/business/coupons"),
  createCoupon: (draft: ApiCouponDraft) =>
    api.post<ApiOwnCoupon>("/v1/business/coupons", draft),
  updateCoupon: (id: string, patch: Partial<ApiCouponDraft>) =>
    api.patch<ApiOwnCoupon>(`/v1/business/coupons/${id}`, patch),
  deleteCoupon: (id: string) => api.delete<void>(`/v1/business/coupons/${id}`),
  /** Hand a draft to moderation. It goes live only once a superadmin approves it. */
  submitCoupon: (id: string) => api.post<ApiOwnCoupon>(`/v1/business/coupons/${id}/submit`),
  /** Pull it back out of the queue, or take an approved one off the shelf. */
  withdrawCoupon: (id: string) => api.post<ApiOwnCoupon>(`/v1/business/coupons/${id}/withdraw`),
  sales: (couponId: string) =>
    api.get<ApiCouponSales>(`/v1/redemptions/coupons/${couponId}/sales`),
};

export const redemptionsApi = {
  /** Look at a code without consuming it — safe to call on every scan. */
  preview: (qr_token: string) =>
    api.post<ApiScanPreview>("/v1/redemptions/preview", { qr_token }),
  /** Actually burn the voucher. Not idempotent by design: a second scan must fail. */
  scan: (qr_token: string, branch_id?: string) =>
    api.post<ApiScanResult>("/v1/redemptions/scan", { qr_token, branch_id }),
  history: (limit = 50) => api.get<ApiRedemption[]>("/v1/redemptions", { limit }),
};
